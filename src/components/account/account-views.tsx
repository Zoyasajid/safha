    ...
    return (
      <Container className="py-24 text-center">
        ...
        <p className="mt-3 text-ink-muted">
          Sign in to see orders, addresses, and saved titles.
        </p>
  ...
  return (
    <Container className="py-12">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3">
        <Link
          href="/account/orders"
          className="rounded-2xl border border-line bg-white/70 p-6 hover:border-gold/50"
        >
    ...
    <Container className="py-12">
      ...
      {!orders.length ? (
        <p className="mt-6 text-ink-muted">
          No orders yet. Your first parcel is a pleasant thing.
        </p>
      ) : (
...
export function OrderDetail({ id }: { id: string }) {
  ...
  if (!order) {
    return (
      <Container className="py-24 text-center">
        ...
        <p className="mt-3 text-ink-muted">
          It may still be saving on this device, or the link is old.
        </p>
  ...
  const paymentLabel =
    order.onlinePaymentMethod === "jazzcash"
      ? "JazzCash"
      : order.onlinePaymentMethod === "easypaisa"
        ? "EasyPaisa"
        : order.onlinePaymentMethod === "card"
          ? "Visa / Mastercard"
          : "online payment";
  const whatsappNumber = SITE.whatsapp.replace(/[^0-9]/g, "");
  const whatsappMessage = encodeURIComponent(
    `Assalam o Alaikum Safha, I placed order ${order.id}. Please share the ${paymentLabel} payment details. Total: ${formatPKR(order.total)}.`,
  );
  return (
    <Container className="py-12">
      <p className="text-sm text-gold">
        Thank you — your Safha parcel is in motion.
      </p>
      ...
      <div className="mt-6 rounded-2xl border border-gold/50 bg-cream-deep p-5">
        <p className="font-medium">
          Order received. Please place your order payment.
        </p>
        <p className="mt-2 text-sm leading-relaxed text-ink-muted">
          {order.paymentMethod === "cod"
            ? "Our team will message you on WhatsApp to confirm your order and delivery details."
            : `Our team will message you on WhatsApp with ${paymentLabel} payment details and confirm your payment.`}
        </p>
        <a
...
export function AddressManager() {
  ...
  return (
    <Container className="grid gap-10 py-12 lg:grid-cols-2">
      ...
      <form
        onSubmit={onSave}
        className="h-fit rounded-2xl border border-line bg-white p-6"
      >
